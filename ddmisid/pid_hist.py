...
class SuperHistFactory: 
    ...
    def book_binning_axes(
        ...
        match binvar:
            case [*_, "P"]:
                axes.append( hist.axis.Variable(binning_dict[binvar], name="P", label=r"$p$ [MeV$/c$]") )
            case [*_, "PT"]:
                axes.append( hist.axis.Variable(binning_dict[binvar], name="PT", label=r"$p_T$ [MeV$/c$]") )
            case [*_, "ETA"]:
                axes.append( hist.axis.Variable(binning_dict[binvar], name="ETA", label=r"$\eta$") )
            case [*_, "nTracks"]:
                axes.append( hist.axis.Variable(binning_dict[binvar], name="nTracks", label=r"\texttt{nTracks}") )
            case [*_, "nSPDhits"]:
                axes.append( hist.axis.Variable(binning_dict[binvar], name="nSPDhits", label=r"\texttt{nSPD}") )
        
        # sanity check
        assert len(axes) == len(binning_dict.keys()), "The number of axes does not match the number of binning variables"
    ...
    def make(
        ...
        super_hpid = hist.Hist(
            hist.axis.StrCategory(["test_calib",], name="calib", label="True Category"),
            hist.axis.StrCategory(["test_reco",],  name="reco", label="Reco Category"),
